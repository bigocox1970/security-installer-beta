  ...
  const fetchComments = async () => {
    try {
      ...
      if (authorsError) {
        ...
        setError('Failed to load comment authors');
        return;
      }
      ...
      const formattedComments = commentsData.map(comment => {
        const author = authorsData?.find(a => a.id === comment.author_id);
        return {
          ...comment,
          author: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authorError) {
        ...
        setError('Failed to load comment author');
        return;
      }
      ...
      <div className="space-y-4">
        {comments.map((comment) => (
          <div key={comment.id} className="flex space-x-4">
            <img
              src={`https://ui-avatars.com/api/?name=${encodeURIComponent(comment.author.full_name)}`}
